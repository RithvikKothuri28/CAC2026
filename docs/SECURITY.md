...
A short-lived server-only `deletedAccounts/{uid}` tombstone blocks writes and backend farm access from already-issued ID tokens after Auth deletion, including when another farm re-adds a stale UID as a member. Its configured Firestore TTL expires after one day; TTL cleanup is asynchronous. Tombstones contain no farm values or email. The Auth deletion trigger enables failure-policy retries, including failures before it creates the durable cleanup job. Quota counters have seven-day TTLs. The scheduler should have alerting for repeated failures before production release.
