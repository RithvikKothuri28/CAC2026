...
void main() {
  ...
  testWidgets('new farmer enters inputs through forms and recovers saved work', (
    tester,
  ) async {
    ...
    Future<void> selectCompatibleCrop(String id) async {
      final finder = find.byKey(ValueKey('field-compatible-$id'));
      await tester.ensureVisible(finder);
      await tester.tap(finder);
      await tester.pumpAndSettle();
    }

    Future<void> selectCurrentCrop(String label) async {
      final finder = find.byKey(const ValueKey('field-current-crop'));
      await tester.ensureVisible(finder);
      await tester.tap(finder);
      await tester.pumpAndSettle();
    ...
    await input('Display name', 'Integration farmer');
    ...
    expect(state.farm!.country, 'United States');
    expect(state.farm!.region, 'Colorado');
    expect(state.farm!.declaredAcres, 10);
    ...
    for (final crop in [
      ...
      // An attractive crop excluded by the farmer must never become a candidate.
      ('Not compatible', '1000', '100', '1', '0'),
    ]) {
      await tap('Add crop');
    ...
    expect(state.farm!.crops, hasLength(3));
    final cornId = state.farm!.crops
        .singleWhere((crop) => crop.name == 'Corn')
        .id;
    final betaId = state.farm!.crops
        .singleWhere((crop) => crop.name == 'Beta fixture')
        .id;
    expect(cornId, isNot('Corn'));
    expect(cornId, matches(RegExp(r'^[0-9a-f-]{36}$')));
    await tap('Fields');
    await tap('Add field');
    ...
    await selectCompatibleCrop(cornId);
    await selectCompatibleCrop(betaId);
    final currentSelector = tester.widget<DropdownButton<String>>(
      find.descendant(
    ...
    expect(
      ...
      unorderedEquals([cornId, betaId]),
    );
    await selectCurrentCrop('Corn');
    await save();
    expect(state.farm!.fields, hasLength(1));
    expect(state.farm!.fields.single.currentCropId, cornId);
    expect(
      state.farm!.fields.single.compatibleCropIds,
      unorderedEquals([cornId, betaId]),
    );

    Future<void> verifyServerCropIds(List<String> compatibleIds) async {
      final saved = await FirebaseFirestore.instance
          .collection('farms')
          .doc(state.farm!.id)
          .get(const GetOptions(source: Source.server));
      final data = saved.data()!['data'] as Map<String, dynamic>;
      final field = (data['fields'] as List).single as Map<String, dynamic>;
      expect(field['currentCropId'], cornId);
      expect(field['compatibleCropIds'], unorderedEquals(compatibleIds));
      expect(
        (data['crops'] as List).any(
          (crop) => crop['id'] == cornId && crop['name'] == 'Corn',
        ),
        isTrue,
      );
    }

    await verifyServerCropIds([cornId, betaId]);

    // Changing a saved field's compatibility is persisted, not recomputed
    // from every crop profile when the edit dialog opens.
    await tester.tap(find.byTooltip('Edit Entered field'));
    await tester.pumpAndSettle();
    await selectCompatibleCrop(betaId);
    await save();
    await verifyServerCropIds([cornId]);
    ...
    expect(state.selected!.plan.assignments.values, everyElement(cornId));
    await navigate('My farm');
    await tester.tap(find.byTooltip('Edit Entered field'));
    await tester.pumpAndSettle();
    await selectCompatibleCrop(betaId);
    ...
    for (final plan in [
      state.optimization!.current,
      ...state.optimization!.pareto,
      ...state.optimization!.representatives.values,
    ]) {
      expect(
        plan.plan.assignments.values,
        everyElement(isIn([cornId, betaId])),
      );
    }
    await tap('Save run');
    ...
    expect(state.scenarioResult!.recommended!.financial.operatingIncome, 1330);
